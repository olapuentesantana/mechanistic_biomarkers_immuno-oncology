log_elastic_net_test <- function(x.test, coef){
  ...
  if  (length(coef) > 1){
    ...
    aux <- exp(t(as.numeric(Intercept) + t(as.numeric(Slope)) %*% t(as.matrix(x.test[,rownames(Slope)]))))
    fit.pred <- aux / (1 + aux)
  }else{
    Slope <- 0
    fit.pred <- as.numeric(Intercept) 
  }
  fit.pred[fit.pred > 0.5] <- "R"
  fit.pred[fit.pred <= 0.5] <- "NR"
  return(fit.pred)
}