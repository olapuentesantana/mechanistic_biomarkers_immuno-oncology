...
###########################################################################################
cross_validation.MT <- function(drug_source, views_source, view_combination, 
                                algorithm, standardize_any=F, standardize_response=F,
                                parameters, k_fold=5, random=NULL) {
  
  # ****************
  # scripts
  source("R/PanCancer_draft_v1/MT_GLM_cluster.R")
  source("R/PanCancer_draft_v1/BEMKL_cluster.R")
  source("R/PanCancer_draft_v1/GLMs_cluster.R")
  ...
  for (k in 1:k_fold){
    ...
    if (algorithm %in% c("Lasso", "Elastic_Net", "Logistic_EN")){
      ...
      output[[k]] <- MT.GLM(drug_source, views_source, view_combination, learning_indices, prediction_indices,
                               standardize_any, standardize_response, parameters = parameters[[algorithm]], iteration = k) 
