...
ObtainIMPRES <- function(gene.tpm){
  ...
  if (any(rownames(gene.tpm) == "VSIR")){
    cat("Gene name changed: C10orf54 instead of VSIR","\n")
    rownames(gene.tpm)[which(rownames(gene.tpm) == "VSIR")] = "C10orf54"
  }
    
  # ***************
  # Gene expression data just for the checkpoint genes
  match_F_1 <- match(as.character(IMPRES.checkpoint.pairs[,1]), rownames(gene.tpm))
  match_F_2 <- match(as.character(IMPRES.checkpoint.pairs[,2]), rownames(gene.tpm))

  if (anyNA(c(match_F_1,match_F_2))){
    warning(paste0("differenty named or missing signature genes : \n", IMPRES_read[!IMPRES_read %in% rownames(gene.tpm)]))
  ...
  IMPRES.matrix <- matrix(0, nrow(IMPRES.checkpoint.pairs), ncol(gene.tpm)) ; colnames(IMPRES.matrix) <- colnames(gene.tpm)
  remove_pairs <- vector("list", length = ncol(gene.tpm)) ; names(remove_pairs) <- colnames(gene.tpm)
  IMPRES.score <- vector("numeric", length = ncol(gene.tpm)) ; names(IMPRES.score) <- colnames(gene.tpm)
