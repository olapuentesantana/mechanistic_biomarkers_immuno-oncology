...
bemkl_supervised_multioutput_regression_variational_train <- function(Km, Y, parameters) {
  ...
  for (o in 1:L) {
    GtimesGT.mu[,,o] <- tcrossprod(matrix(G$mu[,,o], nrow=P), matrix(G$mu[,,o], nrow=P)) + N * G$sigma[,,o]
  }
  ...
  for (iter in 2:parameters$iteration) {
    # cat("\n", iter, "\n")
    # update Lambda
    for (o in 1:L) {
    ...
    for (o in 1:L) {
      ...
      if (P==1){
        upsilon$beta[o] <- 1 / (1 / parameters$beta_upsilon + 0.5 * (GtimesGT.mu[,,o] - 2 * sum(matrix(crossprod(A$mu[,o, drop=F], Km), N, P) * t(matrix(G$mu[,,o], nrow=P))) + sum(KmKm * atimesaT.mu[,,o])))
      }else{
        upsilon$beta[o] <- 1 / (1 / parameters$beta_upsilon + 0.5 * (sum(diag(GtimesGT.mu[,,o])) - 2 * sum(matrix(crossprod(A$mu[,o, drop=F], Km), N, P) * t(matrix(G$mu[,,o], nrow=P))) + sum(KmKm * atimesaT.mu[,,o])))
      }
    ...
    for (o in 1:L) {
      ...
      G$mu[,,o] <- G$sigma[,,o] %*% (upsilon$alpha[o] * upsilon$beta[o] * t(matrix(crossprod(A$mu[,o, drop=F], Km), N, P)) + epsilon$alpha[o] * epsilon$beta[o] * (tcrossprod(be$mu[(L + 1):(L + P)], Y[o,]) - matrix(etimesb.mu[,o], P, N, byrow = FALSE)))
      GtimesGT.mu[,,o] <- tcrossprod(matrix(G$mu[,,o], nrow=P), matrix(G$mu[,,o], nrow=P)) + N * G$sigma[,,o]
      KmtimesGT.mu[,o] <- Km %*% matrix(t(matrix(G$mu[,,o], nrow=P)), N * P, 1)
    }
    # cat(" b ")
    # update gamma
    ...
    for (o in 1:L) {
      ...
      epsilon$beta[o] <- 1 / (1 / parameters$beta_epsilon + 0.5 * as.double(crossprod(Y[o,], Y[o,]) - 2 * tcrossprod(Y[o,], rbind(matrix(1, 1, N), matrix(G$mu[,,o], nrow=P))) %*% be$mu[c(o, (L + 1):(L + P))] + N * btimesbT.mu[o, o] + sum(GtimesGT.mu[,,o] * etimeseT.mu) + 2 * crossprod(rowSums(matrix(G$mu[,,o], nrow=P)), etimesb.mu[,o])))
    }
    # cat(" c ")
    # update b and e
    ...
    if (parameters$progress == 1) {
      ...
      # p(G | A, Km, upsilon)
      for (o in 1:L) {
        lb <- lb - 0.5 * sum(diag(GtimesGT.mu[,,o])) * upsilon$alpha[o] * upsilon$beta[o] + crossprod(A$mu[,o], KmtimesGT.mu[,o]) * upsilon$alpha[o] * upsilon$beta[o] - 0.5 * sum(KmKm * atimesaT.mu[,,o]) * upsilon$alpha[o] * upsilon$beta[o] - 0.5 * N * P * (log2pi - (digamma(upsilon$alpha[o]) + log(upsilon$beta[o])))
      }
      # p(gamma)
      ...
      # p(Y | b, e, G, epsilon)
      for (o in 1:L) {
        lb <- lb - 0.5 * crossprod(Y[o,], Y[o,]) * epsilon$alpha[o] * epsilon$beta[o] + crossprod(Y[o,], crossprod(G$mu[,,o], be$mu[(L + 1):(L + P)])) * epsilon$alpha[o] * epsilon$beta[o] + sum(be$mu[o] * Y[o,]) * epsilon$alpha[o] * epsilon$beta[o] - 0.5 * sum(etimeseT.mu * GtimesGT.mu[,,o]) * epsilon$alpha[o] * epsilon$beta[o] - sum(crossprod(G$mu[,,o], etimesb.mu[,o])) * epsilon$alpha[o] * epsilon$beta[o] - 0.5 * N * btimesbT.mu[o, o] * epsilon$alpha[o] * epsilon$beta[o] - 0.5 * N * (log2pi - (digamma(epsilon$alpha[o]) + log(epsilon$beta[o])))
      }
