...
ggsave(paste0("../figures/PanCancer_draft_v1/TFs/PanCancer_both_alg_TFs_training_performance_SpCorr_1SE_MSE.pdf"),
       width = 10, height = 10)
...
summary_view_features.comb <- subset(summary_view_features, DataType == "TFs")
...
sapply(as.character(unique(summary_view_features$CancerType)), function(X){
  ...
  grid.newpage()
  pdf(paste0("../Figures/PanCancer_draft_v1/TFs/", sapply(strsplit(X, split = "(", fixed = T), head, 1),
             "_TFs_1se_mse_cor_model.pdf"), width = 12, height = 12)
  grid.draw(g)
  dev.off()
...
ggplot(median.features, aes(x = feature, y = CancerType, fill = estimate)) +
  ...
  # ylim(c(-0.1,0.9)) +
  #coord_fixed(ratio = 3) +
  theme(axis.text.x = element_text(size=6, angle = 90, vjust = 0.65), axis.title.x = element_blank(),
...
ggsave(paste0("../Figures/PanCancer_draft_v1/TFs/PanCancer_heatmap_1se_mse_model_cor_tasks.pdf"), width = 12, height = 12)
