...
data$Y[[1]] <- as.matrix(Y[,1])
data$Y[[2]] <- as.matrix(Y[,2])
data$Y[[3]] <- as.matrix(Y[,3])
data$Y[[4]] <- as.matrix(Y[,4])
...
assign("fit",lapply(alpha_range, function(X){
  ...
  cvfit <- cvMTL(data$X , data$Y, type="Regression", Regularization="L21", Lam1_seq = alpha, Lam2=0.8,
